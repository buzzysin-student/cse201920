// Small demo that shows how to use the functions in display.h for drawing the
// beautiful Mandebrot Set.
#include <assert.h>
#include <float.h>
#include <time.h>
#include "display.h"

#define ZOOM 1.3  // zoom-in speed factor applied at navigational adjustment
#define ACC 256    // computational depth, slower and more accurate if larger
#define STEPS \
  1  // stride, use 1 for full resolution, higher numbers for approximations

// Data structure to describe how a small part of the complex plane maps to an
// image. (User of associated functions does not need to know that this
// structure even exists!) Details: s is a scaling factor or scalar target, x is
// the shift in x direction, y is the shift in y direction.
...
void* newFractalState() {
  ...
  *p = (params){2.0, -3.0E-3 * ((rand()) % 1000 - 5.0E2) - 1.5,
                -1.0};  // initialise state
...
// Tilo's implementation for calculating the Mandelbrot Set, an example of a
// fractal. Given a state p that describes how the complex plane maps to an
...
// accuracy, otherwise it returns the iteration at which the recurring
// calculation escaped a theshold.
int calculateMandelbrot(const int accuracy,
                        const int x,
                        const int y,
                        const int w,
                        const int h,
...
bool navigateMandelbrot(display* d, void* data, const char pressedKey) {
  ...
  if ((d == NULL) || (p == NULL))
    return true;  // stop if an input is NULL
  int w = getWidth(d), h = getHeight(d), tx = w / 4,
      ty =
          h / 4;  // image dimensions and target position for interesting pixels
  params c = (params){
      DBL_MAX, 0.0, 0.0};  // buffer to store most interesting visible location
  ...
  for (int y = 0; y < h; y += STEPS) {
    ...
    for (int x = 0; x < w; x += STEPS) {
      ...
      if (i == ACC) {
        // if pixel before was not in the Set and it is close to target, then
        // make it the most interesting visible location
        if (!last) {
  ...
  if (p->s < 1.0E-6)
    p->s = 1.0E-3;
  // use pressedKey input and/or interesting target to update the state p for
  // future drawing
  *p = (params){
      (pressedKey == 120)
          ? (p->s / (ZOOM / 1.15))
      ...
      ((pressedKey == 79) || (pressedKey == 80))
          ? (p->x +
             p->s * (w / 4 - ((pressedKey - 78) * 2 - 3) * (w / 20) - tx) / w)
          : (p->x + p->s * c.x / w),
      ((pressedKey == 81) || (pressedKey == 82))
          ? (p->y +
             p->s * (h / 4 - ((pressedKey - 80) * 2 - 3) * (h / 20) - ty) / h)
          : (p->y + p->s * c.y / h)};
  // indicate that this should be the last time this function is called if ESC
  // is pressed
...
// Run the Demo
int main() {
  display* d = newDisplay("Mandelbrot Flight", 800, 800);
