...
enum GradeValues { G1, G21, G22, G3, Fail, Invalid };
...
int convert(int n, const char mark[])
{
  ...
  // printf("Checking length bounds... => %d\n", checkStringBounds(n, mark));
  if (! checkLengthBounds(n, mark)) return -1;

  for ( int i = n; i > 0; i-- ) {
    char ascii = mark[i-1];
    // printf("Checking ascii bounds (%c)... => %d\n", ascii, checkCharBounds(ascii));    
    if (! checkCharBounds(ascii) ) return -1;
    else result += pwr(10, power++) * (ascii - CHAR_MIN);
  }

  // printf("Checking int bounds (%d)... => %d\n", result, checkIntBounds(result));    
  if (! checkIntBounds(result)) return -1;
  else return result;
}
...
{
  int result;
  if      (m > BOUND_MAX)   result = Invalid;
  else if (m >= BOUND_G1)   result = G1;
  else if (m >= BOUND_G21)  result = G21;
  else if (m >= BOUND_G22)  result = G22;
  else if (m >= BOUND_G3)   result = G3;
  else if (m >= BOUND_MIN)  result = Fail;
  else                      result = Invalid;
  return                    result;
}
...
int main(int n, char *args[n])
{
  ...
  if (n == 1)
    ...
    print(grade(convert(strlen(args[1]), args[1])));
  else
  {
